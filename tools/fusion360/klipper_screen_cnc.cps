/*
Derived from https://github.com/Zergie/mpcnc_post_processor
Adapted for https://github.com/hudrucan/klipper-screen-cnc
MPCNC post processor for Klipper-based CNC milling.
 */

description = "Klipper Screen CNC - Fusion 360";
vendor = "Klipper Screen CNC";
vendorUrl = "https://github.com/hudrucan/klipper-screen-cnc";
...
function onOpen() {
  if (unit != MM) {
    error(localize("Klipper Screen CNC currently requires millimeter output."));
  }
...
function onSection() {
  ...
  if (isFirstSection()) {
    ...
    WriteBlock(
      "START_PRINT"
      , "X=" + xyzFormat.format(jobRanges.x.min) + "," + xyzFormat.format(jobRanges.x.max)
      , "Y=" + xyzFormat.format(jobRanges.y.min) + "," + xyzFormat.format(jobRanges.y.max)
      , "Z=" + xyzFormat.format(jobRanges.z.min) + "," + xyzFormat.format(jobRanges.z.max)
    );
  }
