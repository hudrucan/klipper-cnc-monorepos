<template>
  <v-navigation-drawer
    :key="navigationStyle"
    v-model="naviDrawer"
    :rail="navigationStyle === 'iconsOnly'"
    :rail-width="56"
  ...
  >
    <div class="v-navigation-drawer__image" aria-hidden="true">
      <v-img
        ...
        class="sidebar-background-image"
        style="height: 100%"
        cover
      />
    </div>
    ...
    <OverlayScrollbarsComponent class="nav-scrollbar">
      <v-list class="pr-0 pt-0 ml-0">
        ...
        >
          ...
          />
          <span
            v-if="navigationStyle !== 'iconsOnly'"
            class="text-h6 font-weight-regular text-truncate"
          >
            {{ printerName }}
          </span>
        </v-list-item>
        <sidebar-item
          v-for="(category, index) in visibleNaviPoints"
          :key="index"
          :item="category"
        />
      </v-list>
...
const navigationStyle = computed(
  () => store.state.gui.uiSettings.navigationStyle,
);
...
const boolNaviTemp = computed(
  (): boolean => !isMobile.value && display.mdAndDown.value,
);
...
const printerName = computed((): string => {
  if (store.state.gui.general.printername.length)
    return store.state.gui.general.printername;
...
.sidebar-shell :deep(.v-navigation-drawer__append .v-list-item) {
    padding-inline: 0;
}

.sidebar-shell :deep(.v-navigation-drawer__append .v-list-item__prepend) {
    width: 100%;
    margin-inline-end: 0;
}

.sidebar-shell :deep(.v-navigation-drawer__append .v-list-item__spacer) {
    display: none;
}
